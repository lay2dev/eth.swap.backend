...
import { AddressPrefix, calculateTransactionFee } from '@nervosnetwork/ckb-sdk-utils';
...
export class CkbService extends NestSchedule {
  ...
  async checkDeliverCKB() {
    ...
    for (const deliveredTransfer of deliveredTransfers) {
      const txStatus = (await this.getTxStatus(deliveredTransfer.ckbTxHash)).toString();
  ...
  async deliverCKB(transfer: EthTransfer, isNotify: boolean = true) {
    try {
      ...
      if (capacity < 61 * 10 ** 8) {
        this.logger.error(`capacity ${capacity} is less than 61 CKBytes`, CkbService.name);
        return;
      }
      ...
      const unspentCells = await this.getUnspentCell(lockHash, capacity.toString(), Number(lastId || 0));

      await this.redisService.getClient().set(redisLastIdKey, unspentCells[unspentCells.length - 1].id);
      ...
      const fee = calculateTransactionFee(numberToHex(txSize), numberToHex(this.config.CKB_TX_FEE_RATE));
      ...
      if (isNotify) {
        await this.notificationService.sendSwapResult(transfer, true);
      }
    } catch (err) {
      if (isNotify) {
        await this.notificationService.sendSwapResult(transfer, false);
      }
  ...
  async getUnspentCell(lockHash: string, capacity: string, lastId: number): Promise<CellPlus[]> {
